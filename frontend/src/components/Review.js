...
export default function Review(prop) {
    ...
    const writeDb = async () => {
        ...
        if (reviewData.text !== "" && reviewData.image !== null && reviewData.stars !== 0 && reviewData.item !== "") {
            ...
            const result = await setDoc(doc(database, name), { image: name, stars: reviewData.stars, text: reviewData.text, diningHall: reviewData.diningHall, date: Date(), item: reviewData.item, user: currentUser.currentUser.displayName, upvotes: reviewData.upvotes, userUid: currentUser.currentUser.uid, upvoters: [] });//Add User, Dining hall, Date
            const result_2 = await setDoc(doc(database_all, name), { image: name, stars: reviewData.stars, text: reviewData.text, diningHall: reviewData.diningHall, date: Date(), item: reviewData.item, user: currentUser.currentUser.displayName, upvotes: reviewData.upvotes, userUid: currentUser.currentUser.uid, upvoters: [] });//Add User, Dining hall, Date
